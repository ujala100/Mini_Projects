---- 1. Database
CREATE DATABASE IF NOT EXISTS courier_system_v4;
...
CREATE PROCEDURE calculate_revenue(IN c_id INT)
BEGIN
    ...
    SELECT f.commission_percent, c.price
    INTO commission, price
    FROM franchise f
    JOIN courier c ON f.franchise_id = c.franchise_id
    WHERE c.courier_id = c_id;

    SET f_share = price * commission / 100;
    SET a_share = price - f_share;

    INSERT INTO revenue (courier_id, franchise_earning, admin_earning)
    VALUES (c_id, f_share, a_share);
END$$
...
FOR EACH ROW
BEGIN
    CALL calculate_revenue(NEW.courier_id);
END$$
